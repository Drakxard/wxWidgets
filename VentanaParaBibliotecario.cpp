...
VentanaParaBibliotecario::VentanaParaBibliotecario(wxWindow *parent, Bibliotecario actualBibliotecario) : MyFrameInicioCorrectoBibliotecario(parent) {
	sistema = new System();
	this->Maximize(true);
...
void VentanaParaBibliotecario::MostrarLibros(vector<Libro>aMostrar){
	...
	for (size_t i = 0; i < aMostrar.size(); i++) {
		...
		imagenPortada->Bind(wxEVT_LEFT_DOWN, &VentanaParaBibliotecario::OnLeftUpVerLibro, this);
...
void VentanaParaBibliotecario::OnLeftUpVerLibro(wxMouseEvent& event) {
	int indiceVector = event.GetId() - 2000; 
	if (indiceVector >= 0 && indiceVector < vLibros.size()) {
		DialogoVerLibro *nueva = new DialogoVerLibro(this, vLibros[indiceVector], actualBibliotecario);
...
void VentanaParaBibliotecario::CargarListaSancionados(wxListCtrl* lista){
	lista->DeleteAllItems();
	lista->Freeze();
...
void VentanaParaBibliotecario::CargarListaInfoLibros(wxListCtrl* lista){
	lista->DeleteAllItems();
	lista->Freeze();
	vLibros = sistema->VerContenido<Libro>(sistema->libros(),true);
...
void VentanaParaBibliotecario::CargarListaReservar(wxListCtrl* lista){
	lista->DeleteAllItems();
	lista->Freeze();
...
void VentanaParaBibliotecario::CargarListaEtiquetas(wxListCtrl* lista){
	lista->DeleteAllItems();
	lista->Freeze();
...
void VentanaParaBibliotecario::OnRadioButton_CambiaPestana(wxCommandEvent& event){
	if(m_radio_Libros->GetValue()){
		...
		m_panel_botones_internos->SetSelection(3);
	}				
...
void VentanaParaBibliotecario::OnButtonClickAgregar( wxCommandEvent& event )  {
	int tipo=0;
...
void VentanaParaBibliotecario::OnButtonClickEliminar( wxCommandEvent& event )  {
	long id;
	if(m_radio_InfoLibros->GetValue()){
...
void VentanaParaBibliotecario::OnButtonClickDevolucion( wxCommandEvent& event )  {
	long id = m_list_Alumnos->GetNextItem(-1,wxLIST_NEXT_ALL, wxLIST_STATE_SELECTED);
	if(id != -1){
		if(id >= 0 && id < vAlumno.size()){
...
void VentanaParaBibliotecario::MuestraListaResultadoLibro(wxListCtrl* lista){
	...
	for(int i=0;i<vResultadoLibro.size();i++) { 
		...
		if(vResultadoLibro[i].Existencia()){
			if(vResultadoLibro[i].EstadoDisponibilidad()){
				lista->SetItem(index, 2,"Disponible" );
			}else{
				lista->SetItem(index, 2,"No Disponible" );
			}
...
void VentanaParaBibliotecario::MuestraListaResultado_Autor(wxListCtrl* lista,string palabra){
	if(vResultadoLibro.size()==0){
...
void VentanaParaBibliotecario::MuestraListaResultadoLibro_Disponibilidad(wxListCtrl* lista,bool disponible){
	if(vResultadoLibro.size()==0){
		wxMessageBox("No hay ningun libro con ese nombre","Sin coincidencias",wxOK|wxICON_INFORMATION);
		return;
	}

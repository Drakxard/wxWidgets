	...
	: MyDialogPrestamo(parent), alumnoSeleccionado(alumnoSeleccionado) 
{
	...
	if(m_gridHistorial->GetNumberCols() == 0) {
		...
		m_gridHistorial->SetColLabelValue(2, "Fecha Pr�stamo");
		m_gridHistorial->SetColLabelValue(3, "Fecha Devoluci�n");
	...
	if (m_listaResultadosLibros->GetColumnCount() == 0) {
		...
		m_listaResultadosLibros->AppendTextColumn("T�tulo", wxDATAVIEW_CELL_INERT, 250);
...
void DialogoPrestamo::CargarHistorialEnGrilla() {
	...
	Historial historial;
	int filaActual = 0;
	std::vector<size_t> librosActivosIDs; // Para evitar duplicados en la visualizaci�n
	
	// ---------------------------------------------------------
	// FASE 1: CARGAR PR�STAMOS ACTIVOS (A�n no devueltos)
	// ---------------------------------------------------------
	try {
		...
		for (const auto& prestamo : activos) {
			if (prestamo.id_Alumno == alumnoSeleccionado.VerID()) {
				...
				m_gridHistorial->SetCellValue(filaActual, 4, "EN PR�STAMO");
				
				// Pintar la fila de amarillo claro para resaltar que est� activo
				for (int c = 0; c < 5; c++) {
	...
	try {
		...
		for (const auto& reg : registros) {
			// Verificamos si este registro ya se mostr� como activo para no duplicarlo
			bool esActivo = false;
			...
			if (esActivo) continue; // Si est� activo, saltamos al siguiente
			
			// Si no est� activo, lo agregamos como historial pasado
			m_gridHistorial->AppendRows(1);
...
void DialogoPrestamo::OnBuscarLibro(wxCommandEvent& event) {
	...
	for (Libro& libro : todosLosLibros) {
		...
		if (titulo.Contains(textoBusqueda)) {
			...
			fila.push_back(wxVariant(libro.EstadoDisponibilidad() ? "S�" : "No"));
...
//	
	if (!libroAPrestar.EstadoDisponibilidad()) {
		wxMessageBox("El libro ya est� prestado o no disponible.", "Error", wxICON_ERROR);
		return;
	}
//	
//	 SOLUCI�N 1: Quitamos el "wxString()" de adentro para que el %s no rompa el programa
			...
			activosPrevios = sistema.VerContenido<Libros_en_Prestamo>("Recursos/Binarios/LibrosPrestamosActivos.bin", true); // Ignoramos si el archivo a�n no existe
			...
			wxMessageBox("Pr�stamo registrado exitosamente.");
			EndModal(wxID_OK); 
